...
class TestDBStorage(unittest.TestCase):
    ...
    def test_all(self):
        ...
        objct = self.storage.all()
        self.assertEqual(type(objct), dict)
        self.assertEqual(len(objct), 6)

    @unittest.skipIf(type(models.storage) == FileStorage,
                     "Testing FileStorage")
    def test_all_cls(self):
        """Test all method with specified cls."""
        objct = self.storage.all(State)
        self.assertEqual(type(objct), dict)
        self.assertEqual(len(objct), 1)
        self.assertEqual(self.state, list(objct.values())[0])

    @unittest.skipIf(type(models.storage) == FileStorage,
                     "Testing FileStorage")
    def test_new(self):
        """Test new method."""
        stt = State(name="Washington")
        self.storage.new(stt)
        store = list(self.storage._DBStorage__session.new)
        self.assertIn(stt, store)

    @unittest.skipIf(type(models.storage) == FileStorage,
                     "Testing FileStorage")
    def test_save(self):
        """Test save method."""
        stt = State(name="Virginia")
        self.storage._DBStorage__session.add(stt)
        self.storage.save()
    ...
    def test_delete(self):
        """Test delete method."""
        stt = State(name="New_York")
        self.storage._DBStorage__session.add(stt)
        self.storage._DBStorage__session.commit()
        self.storage.delete(stt)
        self.assertIn(stt, list(self.storage._DBStorage__session.deleted))
